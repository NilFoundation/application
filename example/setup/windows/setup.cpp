...
// service_setup_ex.exe -i --name="My Service" --path="c:\myservice\service.exe" --display="My Service" --description
// "Service Description" service_setup_ex.exe -i --name="My Service" --path="c:\myservice\service.exe" --user=".\Renato
// Tegon Forti" --pass="x1x1x1" service_setup_ex.exe -i --name="My Service" --path="c:\myservice\service.exe"
// --start="manaul" --depends="service1\service2\service3" [check] service_setup_ex.exe -c --name="My Service"
// [uninstallation]
...
public:
    windows_service_setup(application::context &context) : context_(context) {
    }
    ...
    int operator()() {
        ...
        ns::shared_ptr<application::args> myargs = context_.find<application::args>();

        // define our simple installation schema options
        po::options_description install("service options");
        install.add_options()("help", "produce a help message")(",i", "install service")(",c", "check service")(
            ",u", "unistall service")("path", po::value<std::string>(), "service path")(
            "name", po::value<std::string>(), "service name")("user", po::value<std::string>()->default_value(""),
            ...
            "service display name (optional, installation only)")("description",
                                                                  po::value<std::string>()->default_value(""),
                                                                  "service description (optional, installation only)")(
            "start", po::value<std::string>()->default_value("auto"),
            "service start mode (optional, installation only, auto or manaul, default is auto)")(
            "depends",
            po::value<std::string>()->default_value(""),
            "other services of this service depended, multiple services using '\\' separate  (optional, installation "
            "only)");
        ...
        if (vm.count("-i")) {
            ...
            application::example::install_windows_service(application::setup_arg(vm["name"].as<std::string>()),
                                                          ...
                                                          application::setup_arg(vm["depends"].as<std::string>()))
                .install(ec);
        ...
        if (vm.count("-c")) {
            bool exist =
                application::example::check_windows_service(application::setup_arg(vm["name"].as<std::string>()))
                    .exist(ec);
...
int main(int argc, char *argv[]) {
    try {
        ...
        app_context.insert<application::args>(ns::make_shared<application::args>(argc, argv));
