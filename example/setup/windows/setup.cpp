...
public:
    ...
    int operator()() {
        ...
        std::cout << "Setup Windows Service " << "(Note that you need run this AS ADMIN!)" << std::endl;
        ...
        install.add_options()("help", "produce a help message")(",i", "install service")(",c", "check service")(",u",
                                                                                                                "unistall service")(
                "path", po::value<std::string>(), "service path")("name", po::value<std::string>(), "service name")(
                "user", po::value<std::string>()->default_value(""), "user logon (optional, installation only)")("pass",
                                                                                                                 po::value<
                ...
                "service start mode (optional, installation only, auto or manaul, default is auto)")("depends",
                                                                                                     po::value<
                                                                                                             std::string>()->default_value(
                                                                                                             ""),
                                                                                                     "other services of this service depended, multiple services using '\\' separate  (optional, installation only)");
        ...
        if (vm.count("-i")) {
            ...
            application::example::install_windows_service(application::setup_arg(vm["name"].as<std::string>()),
                                                          ...
                                                          application::setup_arg(
                                                                  vm["depends"].as<std::string>())).install(ec);
        ...
        if (vm.count("-c")) {
            bool exist = application::example::check_windows_service(
        ...
        if (vm.count("-u")) {
            application::example::uninstall_windows_service(application::setup_arg(vm["name"].as<std::string>()),
                                                            application::setup_arg(
                                                                    vm["path"].as<std::string>())).uninstall(ec);
