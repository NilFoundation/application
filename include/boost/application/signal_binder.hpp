    ...
    namespace application {
        ...
        public:
            explicit signal_binder(context &cxt) : signals_(io_service_), context_(cxt) {
                signals_.async_wait(boost::bind(&signal_binder::signal_handler, this, boost::asio::placeholders::error,
                                                boost::asio::placeholders::signal_number));
            }

            explicit signal_binder(global_context_ptr cxt) : signals_(io_service_), context_(*cxt.get()) {
                signals_.async_wait(boost::bind(&signal_binder::signal_handler, this, boost::asio::placeholders::error,
                                                boost::asio::placeholders::signal_number));
            }
            ...
            void bind(int signal_number, const handler<> &h) {
                ...
                if (ec) {
                    BOOST_APPLICATION_THROW_LAST_SYSTEM_ERROR_USING_MY_EC("bind() failed", ec);
                }
            }
            ...
            */
            ...
            void unbind(int signal_number) {
                ...
                if (ec) {
                    BOOST_APPLICATION_THROW_LAST_SYSTEM_ERROR_USING_MY_EC("unbind() failed", ec);
                }
            }
        ...
        public:
            ...
            signal_manager(application::context &context) : signal_binder(context) {
                boost::system::error_code ec;

                register_signals(ec);

                if (ec) {
                    BOOST_APPLICATION_THROW_LAST_SYSTEM_ERROR_USING_MY_EC("signal_manager() failed", ec);
                }
            }
