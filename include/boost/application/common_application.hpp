        ...
        public:
            ...
            common(Application &myapp, SignalManager &sm, application::context &context, boost::system::error_code &ec)
                ...
                if (!impl_->get_context().find<run_mode>()) {
                    impl_->get_context().insert<run_mode>(csbl::make_shared<run_mode>(mode()));
                }

                if (!impl_->get_context().find<status>()) {
                    impl_->get_context().insert<status>(csbl::make_shared<status>(status::running));
                }

                if (!impl_->get_context().find<process_id>()) {
                    impl_->get_context().insert<process_id>(csbl::make_shared<process_id>());
                }

                if (!impl_->get_context().find<path>()) {
                    impl_->get_context().insert<path>(csbl::make_shared<path>());
                }
            }
