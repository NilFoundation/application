...
// this is a not official version
        ...
        public:
            ...
            bool lock(boost::system::error_code &ec) {
                ...
                if (named_mutex_ == 0) {
                    named_mutex_.reset(
                        new interprocess::named_mutex(interprocess::open_or_create, instance_id.c_str()));
                }
            ...
            bool lock() {
                ...
                bool result = lock(ec);
                if (ec)
                    BOOST_APPLICATION_THROW_LAST_SYSTEM_ERROR_USING_MY_EC("lock() failed", ec);
        ...
        public:
            ...
            limit_single_instance_default_behaviour(const uuids::uuid &app_uuid) :
                impl_(new limit_single_instance_impl()), uuid_(app_uuid) {
            }
             ...
             */
            limit_single_instance_default_behaviour(const uuids::uuid &app_uuid, const callback &cb) :
                limit_single_instance(cb), impl_(new limit_single_instance_impl()), uuid_(app_uuid) {
            }
